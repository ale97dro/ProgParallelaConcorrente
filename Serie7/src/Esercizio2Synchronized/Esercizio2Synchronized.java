package Esercizio2Synchronized;
...
class ReadWorker implements Runnable {
	...
	public void run() {
		...
		while (Esercizio2Synchronized.isRunning) {
			compareCounter++;
			final StringBuffer sb = new StringBuffer();

			// Build phrase string from shares words
			synchronized(Esercizio2Synchronized.sharedPhrase)
			{
				final Iterator<String> iterator = Esercizio2Synchronized.sharedPhrase.iterator();
...
public class Esercizio2Synchronized {
	...
	public static void main(final String[] args) {
		System.out.println("Esercizio 2 synchronized");
		...
		// Share list
		Esercizio2Synchronized.sharedPhrase = list;
		...
		for (int i = 0; i < 10; i++) {
			...
			{
				Esercizio2Synchronized.sharedPhrase.add(getWord());
			}
			try {
				Thread.sleep(1000);
