package Esercizio2SynCollection;
...
class ReadWorker implements Runnable {
	...
	public void run() {
		...
		while (Esercizio2SynCollection.isRunning) {
			compareCounter++;
			final StringBuffer sb = new StringBuffer();

			// Build phrase string from shares words
			synchronized(Esercizio2SynCollection.sharedPhrase)
			{
				final Iterator<String> iterator = Esercizio2SynCollection.sharedPhrase.iterator();
...
public class Esercizio2SynCollection {
	...
	public static void main(final String[] args) {
		System.out.println("Esercizio2SynCollection");
		...
		// Share list
		Esercizio2SynCollection.sharedPhrase = list;
		...
		for (int i = 0; i < 10; i++) {
				Esercizio2SynCollection.sharedPhrase.add(getWord());
			try {
				Thread.sleep(1000);
