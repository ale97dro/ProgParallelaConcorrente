...
class ReadWorker implements Runnable {
	...
	public void run() {
		...
		while (Esercizio2Base.isRunning) {
			compareCounter++;
			final StringBuffer sb = new StringBuffer();

			// Build phrase string from shares words
			final Iterator<String> iterator = Esercizio2Base.sharedPhrase.iterator();
...
public class Esercizio2Base {
	...
	public static void main(final String[] args) {
		...
		System.out.println("Esercizio 2 Base");
		...
		// Share list
		Esercizio2Base.sharedPhrase = list;
		...
		for (int i = 0; i < 10; i++) {
			Esercizio2Base.sharedPhrase.add(getWord());
			try {
				Thread.sleep(1000);
