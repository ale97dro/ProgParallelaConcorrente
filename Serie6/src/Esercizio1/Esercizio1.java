...
class EventListener 
{
	...
	public static EventListener CostruisciEventListener(final int id, final EventSource eventSource)
	{
		EventListener temp=new EventListener(id);
		// Aggiunge listener alla eventSource per ricevere le notifiche
		eventSource.registerListener(id, temp);
		
		return temp;
	}
	
	//private EventListener(final int id, final EventSource eventSource) 
...
public class Esercizio1 {
	public static void main(final String[] args) 
	{
		...
		System.out.println("Se non stampa, funziona!");
		...
		for (int i = 1; i <= 20; i++)
			...
			allListeners.add(EventListener.CostruisciEventListener(i, eventSource));
